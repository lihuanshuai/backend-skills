...
2. 仅修改任务所需内容，并遵守：
   ...
   - `SKILL.md` frontmatter 的 `name` 必须与目录一致；`description` 必须是一行、无冒号，简短说明能力与准确触发条件，避免罗列实现细节或用宽泛关键词吸引无关任务。
   - 正文使用中文和命令式表达，只保留非显然决策、硬约束、执行入口和输出契约。重复且确定的操作收口到 `scripts/`；按场景把独立的长说明、示例和模板放入 `references/`，在入口说明何时读取，简单技能不必拆分。
