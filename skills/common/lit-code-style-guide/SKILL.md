...
description: 设计、实现或审查随后端发布、无需前端构建链路的 Lit 页面。
---
...
1. 确认 `<repo_dir>` 采用或要求无构建发布；按变更读取相关 route/模板、组件/样式或依赖入口，以及适用的验证配置，不为局部改动遍读全部页面文件。
2. 按场景 reference 修改最小必要范围；沿用现有模板继承、URL 生成、目录命名、认证和 API 约定，不发明平行入口。
3. 按 `<skill_dir>/references/verification-and-review.md` 选择与变更相关的检查并执行仓库必跑项。自动修复或 vendor 更新后审查 diff，重跑受影响检查；已有充分验证且没有新变更、失败或未解决风险时不重复执行。
